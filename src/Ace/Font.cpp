...
namespace ace
{
    ...
    Font::Font() :
        m_ascii(),
        m_it(m_ascii.begin()),
        m_info(std::make_shared<FontInfo>()),
        m_buffer(),
        m_bufferSize(0u),
        m_start(0u),
        m_end(0u),
    ...
    bool Font::Fill(const File& file)
    {
        // Font is placed inside buffer
        m_buffer = file.ReadAll();
        
        m_bufferSize = file.Size();
        ...
        return stbtt_InitFont(
            &m_info->font,
            m_buffer.get(),
            stbtt_GetFontOffsetForIndex(m_buffer.get(), 0)
        );
	...
	Image Font::BakeFontSheet(UInt32 w, UInt32 h, float pixelheight, Int32 first_char, Int32 num_chars)
	{
		...
		CreateGlyphs(m_ascii, cdata, 255, first_char);
		...
		for (UInt32 i = 0u; i < length; ++i)
		{
            ...
            Image Font::BakeTextBox(const char *text_to_print, float lineHeight)
            {
                ...
                for (UInt32 i = 0u; i < length; ++i)
                {
                    ...
                    // Checking characters if they contain  �, �, �, �, �, or �
                    if (character < 0)
                    {
                        character = character + 256;
                    }
                    ...
                    Glyph Font::GetGlyph(const UInt32 letter)
                    {
                        ...
                        return (index < m_end) ? m_ascii[index] : Glyph();
                    }
