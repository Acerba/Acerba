...
const ace::Vertex triangle[6] = {
    { { 0.5f, 0.5f, 0, 0.0f }, { 1.0f, 0.0f }, 0x00FF00FFU },
    { { -0.5f, 0.5f, 0, 0.0f }, { 0.0f, 0.0f }, 0xFF0000FFU },
    { { -0.5f, -0.5f, 0, 0.0f }, { 0.0f, 1.0f }, 0x0000FFFFU },

    { { 0.5f, -0.5f, 0, 0.0f }, { 1.0f, 1.0f }, 0xFF0000FFU },
    { { 0.5f, 0.5f, 0, 0.0f }, { 1.0f, 0.0f }, 0x00FF00FFU },
    { { -0.5f, -0.5f, 0, 0.0f }, { 0.0f, 1.0f }, 0x0000FFFFU },

    //{ { -0.5f, 0.5f,    0, 1.0f }, { 0.0f, 0.0f }, 0xFF0000FFU },
    //{ { 0.5f, 0.5f,        0, 1.0f }, { 1.0f, 0.0f }, 0x00FF00FFU },
    //{ { -0.5f, -0.5f,    0, 1.0f }, { 0.0f, 1.0f }, 0x0000FFFFU },
    //{ { 0.5f, -0.5f,    0, 1.0f }, { 1.0f, 1.0f }, 0xFF0000FFU },
    //{ { 0.5f, 0.5f,        0, 1.0f }, { 1.0f, 0.0f }, 0x00FF00FFU },
    //{ { -0.5f, -0.5f,    0, 1.0f }, { 0.0f, 1.0f }, 0x0000FFFFU },
    //                    
    //{ { -0.5f, 0.5f,    0, 2.0f }, { 0.0f, 0.0f }, 0xFF0000FFU },
    //{ { 0.5f, 0.5f,        0, 2.0f }, { 1.0f, 0.0f }, 0x00FF00FFU },
    //{ { -0.5f, -0.5f,    0, 2.0f }, { 0.0f, 1.0f }, 0x0000FFFFU },
    //{ { 0.5f, -0.5f,    0, 2.0f }, { 1.0f, 1.0f }, 0xFF0000FFU },
    //{ { 0.5f, 0.5f,        0, 2.0f }, { 1.0f, 0.0f }, 0x00FF00FFU },
    //{ { -0.5f, -0.5f,    0, 2.0f }, { 0.0f, 1.0f }, 0x0000FFFFU },
};

//static float your_text_width_calculation(nk_handle handle, float height, const char *text, int len)
//{
//    ace::Font* type = reinterpret_cast<ace::Font*>(handle.ptr);
//    return 8.f;
//};
...
void doStuff(ace::Window& window)
{
...
int main(int, char**)
{
    ...
    ace::GraphicsDevice::BufferData(buffer, 6, triangle, ace::BufferUsage::Static, 5);

    ace::Texture texture1(ace::Image({ "Desert.jpg" }, ace::PixelFormat::RGBA));
    ace::Texture texture2(ace::Image({ "Jellyfish.jpg" }, ace::PixelFormat::RGBA));
    ...
    ace::Material material1(vertex, fragment);
    ace::Material material2(vertex, fragment);
    material2.flags.cullingMode = ace::CullingMode::Both;
    float t = 0;
    ...
    ace::Sprite sprite1;
    ace::Entity e1;
    e1.AddComponent<ace::Sprite>(sprite1);
    e1.AddComponent<ace::Material>(material1);


    ace::Sprite sprite2;
    ace::Entity e2;
    e2.AddComponent<ace::Sprite>(sprite1);
    e2.AddComponent<ace::Material>(material2);

    scene1.GetRoot()->AddChild(e1);
    scene1.GetRoot()->AddChild(e2);
    ...
    /* init gui state */
    ace::UI::Init(window);
    auto& ctx = *ace::UI::GetContext();


    // window.Clear(0x0U);
    // window.Present();

    int MAX_MEMORY = UINT16_MAX;



    std::vector<ace::Font> fonts({ ace::Font(ace::File("arial.ttf")) });
    fonts.begin()->BakeFontSheet(400, 400).WritePNG("roboto.png");
    ace::UI::AddFonts(fonts);

    //struct nk_user_font font;
    //font.userdata.ptr = &vontti;
    //font.height = vontti.m_lineHeight;
    //
    //nk_handle h;
    //font.width = your_text_width_calculation; // (h, vontti.m_lineHeight, "PERKELE", 7);
    ...
    ace::File vile("Roboto-Black.ttf");
    ace::Font vontti(vile);
    ...
    //nkF.height = vontti.m_lineHeight;

    //nk_init_fixed(&ctx, calloc(1, MAX_MEMORY), MAX_MEMORY, &font);
    //nk_init_default(&ctx, &nkF);

    //nk_init_fixed(&ctx, calloc(1, UINT16_MAX), UINT16_MAX, &nkF);
    ...
    {
        ace::Event::Update();
        ace::GraphicsDevice::SetTexture(material1, texture1, "Texture", 0);
        ace::GraphicsDevice::SetTexture(material2, texture2, "Texture", 1);

        const float ratio = window.GetSize().x / window.GetSize().y;
        const float size = 0.5f;
        material1.Uniform("P", ace::Matrix4::Ortho(-size * ratio, size * ratio, -size, size, 0, 10));
        material1.Uniform("V", ace::Matrix4::LookAt({ 0, 0, -1 }, { 0, 0, 0 }, { 0, 1, 0 }));

        material2.Uniform("P", ace::Matrix4::Ortho(-size * ratio, size * ratio, -size, size, 0, 10));
        material2.Uniform("V", ace::Matrix4::LookAt({ 0, 0, -1 }, { 0, 0, 0 }, { 0, 1, 0 }));

        material1.Uniform("M", ace::Matrix4::Translation({ +0.1f, +0.1f, +0.1f }));
        material2.Uniform("M", ace::Matrix4::Translation({ -0.1f, -0.1f, -0.1f }));

        window.Clear(0xBADAFFFFU);

        ace::SpriteManager::Draw(scene1);
